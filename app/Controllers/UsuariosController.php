...
class UsuariosController
{
    ...
    public function edit($withFiles = null)
    {
        try {
            ...
            if(empty($user->getContrasena())){
                $new_user = Usuarios::searchForId($user->getId());//original
                $user->setContrasena($new_user->getContrasena());//editado
            }
            if($user->update()){
        ...
        if (is_array($arrUsuarios) && count($arrUsuarios) > 0) {
            ...
            foreach ($arrUsuarios as $usuario)
                if (!UsuariosController::usuarioIsInArray($usuario->getId(), $params['arrExcluir']))
                    $htmlSelect .= "<option " . (($usuario != "") ? (($params['defaultValue'] == $usuario->getId()) ? "selected" : "") : "") . " value='" . $usuario->getId() . "'>" . $usuario->getDocumento() . " - " . $usuario->getNombre() . " " . $usuario->getApellido() . "</option>";
        }
        $htmlSelect .= "</select>";
        return $htmlSelect;
    }

    private static function usuarioIsInArray($id, $ArrUsuarios)
    {
